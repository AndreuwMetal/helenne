...
import { plainTexture, quiltBump, stripeTexture } from './fabricTextures'
import { buildPouch, type PouchSpec } from './pouchModel'

/**
 * Modelo Azul: medidas y colores estimados a partir de los vídeos
 * (proporción del frente 1,29 : 1; rayas de 2,4 mm).
 */
export function azulSpec(): PouchSpec {
  return {
    width: 22,
    height: 17,
    depth: 12,
    ...
    lining: plainTexture('#c9ced8', 0.8),
    zipper: '#f4f3ef',
  }
}
...
/**
 * Escena de estudio: luz principal suave con sombra, reflejos de un
 * entorno neutro y un suelo invisible que solo recibe la sombra.
 */
export function createPouchView(canvas: HTMLCanvasElement, spec: PouchSpec): PouchView {
  ...
  const frame = (explode: number) => {
    const dist = 78 + 34 * explode
    camera.position.set(0, 18 + 8 * explode, dist)
    camera.lookAt(0, 0.5 + 2 * explode, 0)
  }
  ...
  const draw = () => {
    ...
    pouch.group.position.z = -15 * state.explode
    frame(state.explode)
